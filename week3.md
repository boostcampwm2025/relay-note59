...
### 수행 과정
- Day16 과제에 대해 나와있는 프롬프트를 주고 학습 키워드를 뽑아달라고 했다. 그랬더니 다음과 같이 줬다.
<details> <summary>AI 추출 키워드</summary>
  
### 🧠 핵심 CS 개념
- 벡터(Vector): 수학 및 컴퓨터 과학에서 방향성과 크기를 가지는 데이터 구조로, 텍스트·이미지·음성 등을 고차원 수치로 표현하는 데 사용된다.

- 유사도(Similarity): 두 벡터 간의 관계를 수치화한 값으로, 벡터 검색의 핵심 기준이다. 코사인 유사도, 유클리드 거리 등이 주요 방식이다.

- 거리 함수(Distance Metric): 유사도를 측정하기 위한 함수. 유클리디안 거리, 맨하탄 거리, 코사인 거리 등 다양한 함수가 사용된다.

- 차원의 저주(Curse of Dimensionality): 고차원 데이터에서 발생하는 분산 증가 문제로, 벡터 검색 정확도와 속도 저하의 원인이 된다.

- 최근접 이웃 검색(KNN): 주어진 벡터에 가장 가까운 K개의 이웃 벡터를 찾는 알고리즘으로, 벡터 DB의 핵심 기능 중 하나다.

### 💻 프로그래밍 기법/패턴
- CLI 프로그램 설계: 사용자가 터미널에서 명령어 기반으로 상호작용할 수 있도록 설계하는 방식. 명령 파싱, 인터랙션 흐름 등이 포함된다.

- 직렬화와 역직렬화: 벡터 데이터를 파일에 저장하거나 불러올 때 사용하는 데이터 인코딩/디코딩 방식. JSON, CSV 등 형식 사용 가능.

- 임베딩(Embedding) 기법: 텍스트나 객체를 벡터 공간에 매핑하는 기술로, word2vec은 대표적인 예다.

- 테스트 주도 개발(TDD): 저장/삭제/검색 기능의 정확성을 보장하기 위해 단위 테스트 및 시나리오 테스트를 적용하는 개발 방식.

### 📊 자료구조/알고리즘
- KNN (K-Nearest Neighbors): 유사도 기준으로 가장 가까운 K개의 벡터를 찾는 알고리즘. 선형 탐색(linear scan)부터 KD-tree 등 다양한 방식이 존재.

- 벡터 인덱싱 (Vector Indexing): 빠른 KNN 검색을 위해 사용하는 자료구조. 직접 구현 시 일반적인 정렬 + 거리 비교 방식으로 처리할 수 있음.

- 고차원 배열(Higher-dimensional Array): 다차원 벡터를 저장 및 연산하는 자료구조. 메모리 효율성과 계산 성능 고려 필요.

### 🛠️ 개발 도구/기술
- SQLite (임베디드 DB): 벡터 외 메타데이터(id 등) 저장에 사용할 수 있는 경량 데이터베이스. 벡터 검색 기능은 직접 구현해야 함.

- Node.js 파일 시스템(fs): 벡터 데이터를 JSON/CSV 등의 형식으로 파일에 저장하고 불러오기 위해 필요한 기술.

- Jest / 테스트 프레임워크: 성능 검증, 정확성 평가, 저장/삭제/검색 기능 테스트를 위한 도구.

### 🏗️ 시스템/아키텍처 개념
- 벡터 데이터베이스 구조: 벡터 저장소, 메타데이터 인덱스, 유사도 계산 모듈, 검색 엔진 등으로 구성된 DB 아키텍처.

- 모듈화(Modularization): CLI 인터페이스, 벡터 스토리지, 검색 엔진 등 기능별로 분리하여 유지보수성과 확장성을 확보하는 구조.

- 성능 측정 및 평가: 정확도, 처리 속도, 메모리 사용량 등을 기준으로 시스템 성능을 정량적으로 평가하는 방법론.
...
이렇게 주었고, 이 중 AI가 추천해준 우선순위는 모두 학습했다. 그리고 다음날 체크포인트를 확인해보니, AI가 뽑아준 키워드와 체크포인트가 모두 일치했다.

### AI가 추출한 키워드와 체크포인트 비교

| # | 체크포인트 항목 | 핵심 키워드 일치 여부 | 매칭된 AI 키워드 |
| --- | --- | --- | --- |
| 1 | 벡터의 수학적/공학적 의미 학습 | ✅ | 🧠 벡터(Vector) |
| 2 | 벡터 DB 종류/특징 학습 및 사용 | ✅ | 🛠️ 유명한 벡터 DB, 🏗️ 벡터 DB 구조 |
| 3 | N차원 벡터 메모리 저장 구현 | ✅ | 📊 고차원 배열, 🏗️ 벡터 DB 구조 |
| 4 | 벡터 컬렉션 구성 및 탐색 구현 | ✅ | 📊 KNN, 유사도 탐색 |
| 5 | 유사도 검색 알고리즘 학습/비교 | ✅ | 🧠 유사도, 💻 거리 함수 |
| 6 | 디스크 기반 벡터 DB 구현 | ✅ | 🛠️ Node.js 파일 시스템, SQLite |
| 7 | 벡터 저장 시 효율성/안정성 고려 | ✅ | 🛠️ 직렬화/역직렬화, 모듈화 |
| 8 | 벡터 DB에서 삭제/업데이트 구현 | ✅ | 🏗️ CRUD 지원 벡터 저장소 |
| 9 | 메모리보다 큰 DB도 처리 가능 | ✅ | 🏗️ 디스크 저장 구조, 파일 기반 DB |
| 10 | 관계형/전문/벡터 DB 비교 학습 | ✅ | 🧠 벡터 DB vs RDB |
| 11 | 임베딩 방법 학습 | ✅ | 💻 임베딩 기법, word2vec |
| 12 | API/라이브러리로 벡터 변환 실습 | ✅ | 🛠️ word2vec API 적용 |
| 13 | 직접 임베딩 모델 구현 | ✅ | 💻 임베딩 모델 구현 |
| 14 | 벡터 DB 기반 앱 구현 | ✅ | 💻 CLI 앱 구현, 응용 프로그램 |

### 수행후기
이번 과제에서 AI를 활용해 핵심 키워드를 미리 추출하고 나서 학습을 시작하니, 전체적인 방향성과 중요 포인트를 놓치지 않고 준비할 수 있었다.
특히 내가 할 때는 항상 빠뜨리는 개념이나 놓치는 부분이 있었는데, 이번에는 AI가 추출한 키워드가 실제 체크포인트와 100% 일치해서 매우 놀랍고 만족스러웠다.
이런 경험을 통해, 앞으로 과제를 시작할 때 단순히 문제만 읽고 바로 들어가는 것이 아니라, AI 키워드 추출을 루틴으로 활용하면 정말 도움이 되겠다는 확신이 들었다.
정리된 키워드를 기반으로 학습의 우선순위를 명확히 세울 수 있어서, 더 전략적으로 공부할 수 있었고, 막연한 불안감도 줄어들었다.
앞으로도 활용하면 좋을 것 같다.
